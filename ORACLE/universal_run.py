...
def main() -> int:
    ...
    parser.add_argument("--preset", default="clean_realistic", choices=["silver_gray", "dark", "warm", "viral_hdr", "clean_realistic"])
    ...
    state["target"]["compositing_preset"] = args.preset
    save_state(state_file, state)
    ...
    f09_input = base / "F09_AETHER_COMPOSITUM" / f"{args.campaign_id}_f08_output.mp4"
    f09_input.parent.mkdir(parents=True, exist_ok=True)
    run(["modal", "volume", "get", args.video_volume, current_input, str(f09_input)], args.dry_run)
    f09_output = base / "F09_AETHER_COMPOSITUM" / f"{args.campaign_id}_f09_composited.mp4"
    f09_report = base / "F09_AETHER_COMPOSITUM" / "stage_report.json"
    f09_command = [sys.executable, "F09_LUTHER/CODEBASE/run_stage.py",
                   "--input", str(f09_input), "--output", str(f09_output),
                   "--report", str(f09_report), "--preset", args.preset]
    if args.dry_run:
        f09_command.append("--mode")
        f09_command.append("contract")
    stdout = run(f09_command, args.dry_run)
    report = json.loads(stdout.strip().splitlines()[-1]) if stdout and stdout.strip() else {
        "status": "PLANNED", "stage": "F09_AETHER_COMPOSITUM", "preset": args.preset,
    }
    report_path = write_stage_report(base, "F09_AETHER_COMPOSITUM", report)
    state["completed_stages"].append("F09_AETHER_COMPOSITUM")
    state["artifacts"]["F09_AETHER_COMPOSITUM"] = str(report_path)
    current_input = str(f09_output.resolve()) if not args.dry_run else current_input
    state["artifacts"]["latest_f09"] = current_input
    state["current_stage"] = "F10_CUSTOS_RESTITUTIO"
    save_state(state_file, state)

    # ── F10 CUSTOS RESTITUTIO (local download + seal) ──
